...
/* ── DATA – same as original, unchanged ────────────────────────────────── */
const tmPinTypes = {
  ...
  town:        { color:'#41087d', icon:'fa-solid fa-mountain-city',       label:'Town'             },
...
const tmAllMarkers = [];
const tmActiveFilters = new Set();
let tmLastGalleryCardId   = null;
let tmLastGalleryCardName = '';
...
/* ── LEGEND & FILTERING (unchanged logic, but uses efficient Set) ── */
(function tmBuildLegend() {
  ...
  Object.entries(tmPinTypes).forEach(([key, t]) => {
    ...
    row.innerHTML = `<div class="tm-legend-pill" style="background:${t.color}"><i class="${t.icon}"></i></div><span>${t.label}</span>`;
  ...
  if (toggleBtn) {
    if (window.innerWidth <= 768) legendEl.classList.add('tm-legend--collapsed');
    toggleBtn.addEventListener('click', () => legendEl.classList.toggle('tm-legend--collapsed'));
  }
  tmUpdateBadge();

  /* Mobile FAB + Sheet – same as before but without backdrop-filter */
  ...
  function tmOpenSheet() { sheetEl.classList.add('tm-sheet--open'); document.body.style.overflow = 'hidden'; }
  function tmCloseSheet() { sheetEl.classList.remove('tm-sheet--open'); document.body.style.overflow = ''; }

  function tmSyncBadges() {
    const n = tmActiveFilters.size;
    const txt = n === 0 ? 'all shown' : `${n} active`;
    if (badgeEl) badgeEl.textContent = txt;
  ...
  if (fabEl && sheetEl && sheetListEl) {
    Object.entries(tmPinTypes).forEach(([key, t]) => {
      ...
      function tmSyncRowState(active) {
        ...
        [row, desktopRow].forEach(r => { if(r) r.classList.toggle('tm-legend-active', active); });
      }
      row.addEventListener('click', () => {
    ...
    sheetEl.addEventListener('click', e => { if(e.target === sheetEl) tmCloseSheet(); });
    if (sheetReset) {
...
/* ── MAP HEIGHT – optimised: only invalidate when size actually changes ── */
function tmFitHeight() {
  ...
  const newHeight = Math.max(minH, vh - used);
  if (Math.abs(shell.offsetHeight - newHeight) > 5) {
...
function tmDebouncedFitHeight() {
  ...
  tmResizeTimer = setTimeout(tmFitHeight, 200);
}
...
(function tmBackButton() {
  ...
  backBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      const freshCard = document.querySelector(`.tm-gc-card[data-card-id="${tmLastGalleryCardId}"]`);
      if (freshCard) {
...
/* ══════════════════════════════════════════════════════════════
   GALLERY – OPTIMISED: debounced search, event delegation
══════════════════════════════════════════════════════════════ */
let tmGalleryQuery      = '';
...
function tmBuildGalleryFilters() {
  ...
  if (countryChips) {
    tmCountries.forEach(c => {
      ...
      btn.innerHTML = `<div class="tm-gf-flag-wrap"><span class="fi fi-${c.flag}"></span></div><span class="tm-gf-chip-label">${c.name}</span>`;
      btn.addEventListener('click', () => {
  ...
  if (typeChips) {
    Object.entries(tmPinTypes).forEach(([key, t]) => {
      ...
      btn.innerHTML = `<div class="tm-gf-icon-circle" style="background:${t.color}"><i class="${t.icon}"></i></div><span class="tm-gf-chip-label">${t.label}</span>`;
      btn.addEventListener('click', () => {
...
function tmGalleryPhotoGrid(imgs) {
  ...
  if (n === 1) return `<div class="tm-gc-photos tm-gc-photos--1"><div class="tm-gc-ph" data-idx="0"><img src="${th(imgs[0])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[0])}'"></div></div>`;
  if (n === 2) return `<div class="tm-gc-photos tm-gc-photos--2"><div class="tm-gc-ph" data-idx="0"><img src="${th(imgs[0])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[0])}'"></div><div class="tm-gc-ph" data-idx="1"><img src="${th(imgs[1])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[1])}'"></div></div>`;
  if (n === 3) return `<div class="tm-gc-photos tm-gc-photos--3"><div class="tm-gc-ph" data-idx="0"><img src="${th(imgs[0])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[0])}'"></div><div class="tm-gc-ph" data-idx="1"><img src="${th(imgs[1])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[1])}'"></div><div class="tm-gc-ph" data-idx="2"><img src="${th(imgs[2])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[2])}'"></div></div>`;
  if (n === 4) return `<div class="tm-gc-photos tm-gc-photos--4"><div class="tm-gc-ph" data-idx="0"><img src="${th(imgs[0])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[0])}'"></div><div class="tm-gc-ph" data-idx="1"><img src="${th(imgs[1])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[1])}'"></div><div class="tm-gc-ph" data-idx="2"><img src="${th(imgs[2])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[2])}'"></div><div class="tm-gc-ph" data-idx="3"><img src="${th(imgs[3])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[3])}'"></div></div>`;
  const extra = n - 5;
  const tiles = imgs.slice(1, 5).map((name, i) => {
    const isOverflow = extra > 0 && i === 3;
    return `<div class="tm-gc-ph" data-idx="${i+1}"><img src="${th(name)}" loading="lazy" alt="" onerror="this.src='${tmFull(name)}'">${isOverflow ? `<div class="tm-gc-more">+${extra+1}</div>` : ''}</div>`;
  }).join('');
  return `<div class="tm-gc-photos tm-gc-photos--5plus"><div class="tm-gc-ph tm-gc-ph--hero" data-idx="0"><img src="${th(imgs[0])}" loading="lazy" alt="" onerror="this.src='${tmFull(imgs[0])}'"></div><div class="tm-gc-ph-strip">${tiles}</div></div>`;
}
...
function tmRenderGallery() {
  ...
  tmCountries.forEach(country => {
    ...
    const matchingCities = country.cities.filter(city => {
      ...
      const matchesSearch = !q || country.name.toLowerCase().includes(q) || city.name.toLowerCase().includes(q);
  ...
  sections.forEach(({ country, cities }) => {
    ...
    header.innerHTML = `<span class="tm-gc-flag fi fi-${country.flag}"></span><h3 class="tm-gc-country-name">${country.name}</h3><span class="tm-gc-country-desc">${country.desc}</span>`;
    ...
    cities.forEach(city => {
      ...
      const typeBadgesHtml = city.types.map(({ type }) => {
        const t = tmPinTypes[type] || tmPinTypes.default;
        return `<div class="tm-gc-type-badge" style="color:${t.color};background:${t.color}14;border-color:${t.color}30"><i class="${t.icon}"></i> ${t.label}</div>`;
      }).join('');
      ...
      info.innerHTML = `
        ...
        <button class="tm-gc-loc-name" data-lat="${primaryLat}" data-lng="${primaryLng}" style="background:none;border:none;padding:0;margin:0;box-shadow:none;outline:none;border-radius:0;">${city.name} <i class="fa-solid fa-map-location-dot"></i></button>
  ...
  /* Event delegation for gallery card clicks (zoom + map fly-to) */
  container.querySelectorAll('.tm-gc-card').forEach(card => {
    card.querySelectorAll('.tm-gc-ph').forEach(ph => {
      ph.style.cursor = 'zoom-in';
      ph.addEventListener('click', function(e) {
        e.stopPropagation();
        const idx = parseInt(this.dataset.idx) || 0;
        const cityName = card.querySelector('.tm-gc-loc-name').innerText.split(' ')[0];
        const imgs = card.__tmImgs; // we need to store fullSrcs on card
        if (!card.__tmImgs) {
          const cityEntry = cities.find(c => c.name === cityName);
          if (cityEntry) card.__tmImgs = cityEntry.imgs.map(name => tmFull(name));
        }
        if (card.__tmImgs) tmOpenLb(card.__tmImgs, idx, cityName);
      });
    });
    const locBtn = card.querySelector('.tm-gc-loc-name');
    if (locBtn) {
      locBtn.addEventListener('click', function() {
        const lat = parseFloat(this.dataset.lat);
        const lng = parseFloat(this.dataset.lng);
        tmLastGalleryCardId   = card.dataset.cardId;
        tmLastGalleryCardName = card.querySelector('.tm-gc-loc-name').innerText.split(' ')[0];
        ...
        if (backBtn) {
          ...
          if (nameEl) nameEl.textContent = tmLastGalleryCardName;
        }
        setTimeout(() => {
    ...
    // Store full image array on card for lightbox
    const cityName = card.querySelector('.tm-gc-loc-name')?.innerText.split(' ')[0];
    const cityData = cities.find(c => c.name === cityName);
    if (cityData) card.__tmImgs = cityData.imgs.map(name => tmFull(name));
  });
}
